...
1.  Start the development server with file watching and live-reload: `lein run -- -d example -s -c -b firefox`.
...
4.  For a final production build, stop the server and run `lein run -- -d example`.
5.  To validate the final build, run `lein run -- -d example -l` to perform a link check.
