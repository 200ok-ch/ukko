...
typographic rules: keys (aka. properties) will be written in italics,
values will be written in code blocks (monospaced font). Explanations
wil generally use the default values with the corresponding key in
